...
| R-COEF | 기체 계수 파라미터화(재타깃) | `fdm_coef`(38필드, `fdm.c`) | `cov-driver`(질량↑→침하) + `fmi-check`(SetReal 질량×1.5 효과) + 구조체↔channels.json 순서 게이트 | Native fmu-check/coverage |
...
| **배포 코어 구조적 커버리지** | **E** | **`fdm.c` 라인 커버리지 100%** (라인 수는 gcov 구현별 상이: gcc 205 / clang 210) — `golden-check` + `cov-driver` 결합, 임계값 100% CI 게이트 (`make -C native coverage`) |
...
| 모델 구조적 커버리지 분석(MC/DC) | E* | **C 배포 코어 `fdm.c` MC/DC 조건 커버리지 = 98.7% (76개 중 75)**, gcc-14 `-fcondition-coverage` + `gcov --conditions`로 계측, CI 강제 (`make -C native mcdc`). 미커버 1개(L71)는 §5 정당화된 도달불가 방어코드. **\* 계측 도구(gcc)는 DO-330 툴 적격성 미인정 → 인증 credit 청구는 불가(§6-#1)** |
...
- **구조적(라인) 커버리지 — CI 강제**:
  - 배포 C 코어 `fdm.c` = **100% (205/205 라인)**, gcov 계측 (`make -C native coverage`).
...
- **MC/DC(수정된 조건/판정) 커버리지 — CI 강제**:
  - 배포 C 코어 `fdm.c` = **98.7% 조건 커버리지 (76개 중 75)**, gcc-14
    `-fcondition-coverage` + `gcov --conditions` 계측 (`make -C native mcdc`).
  - **도달 가능한 전 조건 = 100% 커버**. `golden-check`(참조 궤적)가 정상 경로를,
    `cov-driver`(assertion 기반)가 경계·폴트 판정(고도 클램프, 편각/스로틀 한계, 서보
    jam/floating/slow, 지상 sink/friction/자세 클램프, asin 가드, 큰-스텝 클램프)을 구동.
  - **정당화된 예외 1건 (도달불가 방어코드)**: `fdm.c` L71 `quat_normalize`의
    `if (n == 0.0)` — 유효한 자세 쿼터니언은 노름이 0이 될 수 없으므로(적분이 노름을
    보존, 부팅 상태도 단위 노름) 이 나눗셈 보호 가드의 참(true) 측은 유효 상태로 도달
    불가. DO-178C §6.4.4.3(구조적 커버리지 분석의 미달 해소)이 허용하는 **분석적 정당화**.
    MC/DC 게이트(`mcdc.sh`)는 이 라인만 허용목록에 두고 그 외 미커버 조건은 빌드 실패.
...
2. **MC/DC 커버리지**: 라인 커버리지(배포 C 100%·JS 모델 100%)와 **MC/DC 조건 커버리지
   (배포 C 코어 98.7%, 도달가능 조건 100%, 정당화 예외 1건)** 모두 계측·CI 강제됨
   (gcc-14 `-fcondition-coverage`). **잔여 갭은 *측정*이 아니라 *도구 적격성*** — 아래 #1
   참조. gcc의 커버리지 계측이 DO-330으로 적격성 인정되지 않는 한, 이 MC/DC 수치는
   강력한 검증 근거이나 인증 credit으로 직접 청구할 수 없다. (JS 참조 모델의 MC/DC는
   node 내장 도구가 조건 커버리지를 산출하지 않아 미측정 — 단, 배포 산출물은 C 코어이며
   그에 대한 MC/DC는 확보됨.)
